...
public class RowListView2 {
    ...
    private HashMap<ListItemView, Integer> listItemGoogleNewsMap;
    private PlaceHolderView newsListView;

    public RowListView2(PlaceHolderView newsListView) {
    ...
    public void onResolved(){
        mainListView.getBuilder()
                ...
                .setLayoutManager(new StaggeredGridLayoutManager(
                        listItemGoogleNewsMap.size()  > 0 ? listItemGoogleNewsMap.size() : 1,
                        LinearLayoutManager.HORIZONTAL));

        for(ListItemView listItemView : listItemGoogleNewsMap.keySet()){
            switch (listItemGoogleNewsMap.get(listItemView)){
                case DO_NOTHING:
                    break;
                case ADD_VIEW:
                    mainListView.addView(listItemView);
                    break;
                case UPDATE_VIEW:
                    mainListView.refreshView(listItemView);
                    break;
            }
        }
    }
                    ...
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        ...
                        public void run() {
                            for(GoogleNews.entry entry : googleNews.getResponse().getFeedData().getEntryList()){
                                listItemGoogleNewsMap.put(new ListItemView(entry), ADD_VIEW);
                            }
                            newsListView.refreshView(RowListView2.this);
                        }
                    });
